...
After a successful push, `axi run` waits for the run created by that push; only expiry of that short trigger window falls back to an IPC rerun. A failure while taking the pre-push run baseline or polling for the triggered run is returned as an error instead of pushing without a baseline or starting a possible duplicate rerun.
