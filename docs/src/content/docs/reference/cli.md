...
After a successful push, `axi run` waits for the run created by that push; only expiry of that short trigger window falls back to an IPC rerun. It records a pre-push run baseline before the push and rechecks it if the submitted head changes while the inspection is in flight. A failure in either baseline lookup or in polling for the triggered run is returned as an error instead of pushing without a baseline or starting a possible duplicate rerun; when both the push and poll fail, the returned error preserves both causes. If polling fails after the push reached the gate, inspect `no-mistakes axi status` before retrying because the run may already exist.
