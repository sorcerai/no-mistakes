...
- Reports a defect class once: a defect finding is anchored to one primary file and line and enumerates, in that same description, every other place in the changed code where the same invariant is violated or must hold (another axis, direction, or representation; a sibling call path, command, action, or state transition; another consumer of the same input, field, or record), each as file:line, and an incomplete-validation finding lists every consumed field still unvalidated, so one fix round can close the class instead of the next review reporting the sibling the fix left behind
...
- Runs every review turn - the initial review and every full rereview - as a fresh, session-free invocation, so the rereview that certifies a fix round never resumes the session whose findings prescribed those fixes; the rereview prompt additionally reframes fix-round changes as pipeline-authored code to review under the same adversarial standard as the author's changes, with prior findings, fix summaries, and same-round tests treated as claims rather than evidence; when a defect it reports is in code a prior fix round changed, or is a sibling site of an invariant a prior fix round addressed, it says so in the description, naming the round and whether that fix introduced the defect, left the sibling behind, or moved the defect, and lists every remaining sibling site; and when the defects it finds sit in prior-fix-round code that exceeds what the original finding required, it reports a single `ask-user` finding recommending that round be reverted to the minimal fix instead of filing further repairs on that code
...
- With opt-in [`jev.review_assist`](/no-mistakes/reference/global-config/#jev) and `TYPESAFE_API_KEY` set, runs one batched TypeSafe Jev evaluation before each review turn and adds its typed answers to the review prompt as advisory input only: a ranked surrounding-context reading list (candidates enumerated in code). The assist can only ever add to the prompt - never remove a file, clause, or obligation - complete-change coverage and the `reviewed_paths` contract are unchanged, the reviewer stays a fresh session-free invocation, and every failure mode falls back to the same cold complete review
...
The fixer's unit of work is the invariant a finding violates, not the reported instance: before editing it states that invariant and enumerates every place in the changed area where it must hold (every axis, direction, and representation; every sibling call path, command, action, and state transition; every consumer of the same input, field, or record), then closes all of them in the same round with the same small correction or at the one shared boundary that makes them all hold, because a fix that closes only the reported site and leaves a sibling reachable is reported again by the next review.
...
- Bounds those agent turns with [`test_agent_timeout`](/no-mistakes/reference/global-config/#test_agent_timeout): each evidence-gathering or Test-repair invocation gets its own budget. An expired budget cancels the agent and parks for a decision with an ask-user finding rather than failing the run as a code defect. The configured baseline's result from the same execution stays on that park; after a cut repair turn the baseline runs once more first, so the park reports the state the cut left. A cut fix round also keeps the findings of the gate it was answering, selected or not, and the last completed evidence turn's verdict and scenarios. A late successful return after the deadline is not used as a Test pass. A commit the timed-out agent already made is recorded locally for custody, unless an unfinished rebase or merge leaves only a partial HEAD. While the worktree holds uncommitted changes or commits past the head the last completed evidence turn saw (before one completes, past the head the first cut measured from, which each later park carries forward and measures again), the park names them with the commands to inspect them and refuses approval, because Document and Push would commit and publish them; a fix response validates them. Otherwise approving that park is a Test exception, not a silent green pass. A fix response whose selection holds only the budget-cut findings re-runs validation without a repair turn. Raise `test_agent_timeout` when targeted tests or evidence gathering routinely approach the default 30m; the shipped default is a stall bound, not slack for a long suite.
