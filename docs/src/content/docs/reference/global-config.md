...
Codex receives that run's exact evidence directory through `--add-dir`, including resumed invocations, so `workspace-write` permits evidence collection without granting the parent directory or sibling runs. This does not change the operator's sandbox mode or remove their additional writable roots.
